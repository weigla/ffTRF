...
- transfer-function magnitude/phase plots and observed-vs-predicted coherence diagnostics
...
- optional multi-taper fitting with transfer-function and coherence diagnostics
...
### Multi-Taper Diagnostics

`examples/example_multitaper_diagnostics.py`

![Multi-taper diagnostics example](docs/images/examples/multitaper_diagnostics.png)
...
usually more stable spectral estimates, switch to `spectral_method="multitaper"`:
...
In multi-taper mode, DPSS tapers replace the ordinary segment window, so
`window` should stay `None`.

### Transfer-Function And Coherence Diagnostics

The fitted model always stores the complex transfer function. You can inspect
it directly or compare predictions and observations in the frequency domain:
...
fig_tf, axes_tf = model.plot_transfer_function()
...
`diagnostics.observed_spectrum` expose the matching output spectra.
