...
def test_bayesian_trf_recovers_kernel() -> None:
    ...
    model = BayesianTRF(direction=1, metric="pearsonr")
    trained = model.train(
        stimulus,
        response,
        fs=fs,
    ...
    assert result.segment_duration == pytest.approx(1.024)
...
def test_bayesian_trf_matches_time_domain_ridge_for_fixed_lambda() -> None:
...
def test_bayesian_trf_cross_validates_regularization() -> None:
...
def test_bayesian_trf_decay_ridge_suppresses_late_lags() -> None:
...
def test_bayesian_trf_ard_downweights_irrelevant_features() -> None:
...
def test_bayesian_trf_ard_requires_evidence_mode() -> None:
...
def test_bayesian_trf_backward_api_and_wrapper() -> None:
    ...
    result = model.result()
    wrapped_prediction, wrapped_score = predict_bayesian_trf(
        stimulus,
        result,
        response=response,
        average=True,
    )
    assert isinstance(result, BayesianTRFResult)
    assert isinstance(wrapped_prediction, list)
    assert wrapped_score > 0.75


def test_bayesian_trf_plot_methods_if_matplotlib_available() -> None:
...
def test_bayesian_trf_supports_custom_credible_levels() -> None:
    ...
    interval_95, _ = model.credible_interval_at(0.95)
...
def test_bayesian_trf_rejects_invalid_credible_levels() -> None:
...
def test_bayesian_trf_save_load_and_copy_roundtrip(tmp_path: Path) -> None:
    ...
    path = tmp_path / "bayesian.pkl"

    prediction, score = model.predict(stimulus=stimulus[-1], response=response[-1])
    model.save(path)

    restored = BayesianTRF(direction=1)
    restored.load(path)
    ...
    assert np.allclose(restored.posterior_std, model.posterior_std, rtol=1e-10, atol=1e-12)
...
def test_top_level_aliases_export_bayesian_api() -> None:
    assert BayesianFrequencyTRF is BayesianTRF

    rng = np.random.default_rng(42)
    stimulus = rng.standard_normal(1_024)
    response = rng.standard_normal((1_024, 1))

    result = fit_bayesian_frequency_trf(
        stimulus,
        response,
        fs=1_000,
        tmin=0.0,
    ...
    prediction = predict_bayesian_frequency_trf(stimulus, result)
