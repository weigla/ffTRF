"""Bayesian frequency-domain TRF fitting.
...
class BayesianTRFResult:
    """Posterior summary for the Bayesian TRF estimator.
    ...
    credible_level:
        Posterior mass used when constructing :attr:`credible_interval`.
    direction:
    ...
    def plot(
        ...
        *,
        ...
        ----------
        credible_level:
            Optional posterior mass for the interval. If omitted, the level
            stored on the result is used.
        """

        from .plotting import plot_kernel

        weights, times = _slice_result_weights(self, tmin=tmin, tmax=tmax)
        credible_interval = _slice_result_credible_interval(
            self,
            tmin=tmin,
            tmax=tmax,
    ...
    def credible_interval_at(
        ...
        *,
        ...
        """Return a marginal credible interval for an arbitrary credible level.

        Parameters
        ----------
        credible_level:
            Posterior mass to use when constructing the interval. This can be
            different from the level stored on the result.
        tmin, tmax:
            Optional lag window in seconds.
        """
...
class BayesianFrequencyTRF:
    ...
    - call :meth:`plot` to visualize the posterior mean kernel
    ...
    - ``"ridge"``: one global shrinkage value per output. This is the closest
      Bayesian analogue of classic ridge / mTRF regularization.
    - ``"smooth"``: finite-difference smoothness prior over lags. Neighboring
      coefficients are encouraged to vary gradually, which is useful when you
      expect broad TRF structure rather than isolated spikes.
    - ``"decay_ridge"``: ridge prior with stronger shrinkage at later lags.
      This expresses the belief that the response should be concentrated early
      in the latency window, which is often sensible for ABR-style work.
    - ``"ard"``: automatic relevance determination with one evidence-updated
      precision per input feature. Entire regressor kernels can be shrunk
      strongly when the data suggest that a feature is irrelevant.
    ...
    - ``ridge``: no prior-specific parameter is required.
    - ``smooth``: ``smoothness_order`` controls how strongly roughness is
      penalized. Order 1 penalizes slope changes less strongly than order 2.
    - ``decay_ridge``: ``decay_tau`` is the key parameter. Smaller values make
      the prior concentrate more strongly on early lags.
    - ``ard``: no extra prior-specific parameter is required, but
      ``regularization`` must stay ``None`` because ARD learns separate
      shrinkage values from evidence.
    """
    ...
    def train(
        ...
        *,
        ...
        - ``"smooth"`` for smooth kernels via finite differences
        ...
        - ``smoothness_order`` matters only for ``prior="smooth"``. Higher
          values impose a stronger preference for slowly varying kernels.
        - ``decay_tau`` matters only for ``prior="decay_ridge"``. It is a time
          constant in seconds that controls how quickly prior variance decays as
          lag increases. Smaller values concentrate the fit more strongly on
          early lags.
        - ``alpha_init`` and ``beta_init`` are initialization values for the
          evidence updates. They rarely need careful tuning, but they are most
          relevant for ``"ard"`` because separate feature-wise shrinkage values
          are learned from them.
        - ``credible_level`` sets the default posterior mass for the stored
          marginal credible interval. For example, ``0.95`` gives a 95%
          interval and ``0.8`` gives an 80% interval.

        In practice:

        - use ``ridge`` when you want the most direct comparison to non-Bayesian
          ridge fits
        - use ``smooth`` when you expect a broad, smooth TRF
        - use ``decay_ridge`` when early latencies should dominate
        - use ``ard`` when you have multiple regressors and want the model to
    ...
    def plot(
        ...
        *,
        ...
        """Plot one posterior mean kernel, optionally with a credible interval.

        Parameters
        ----------
        credible_level:
            Optional posterior mass for the interval. If omitted, the level
            stored on the estimator is used.
        """
        ...
        if show_credible_interval:
            credible_interval = _slice_credible_interval(
                self.posterior_std,
                self.weights,
                self.times,
                tmin=tmin,
    ...
    def credible_interval_at(
        ...
        *,
        ...
        credible_level:
            Posterior mass to use when constructing the interval. This can be
            different from the level stored on the estimator.
        tmin, tmax:
            Optional lag window in seconds.
        """
